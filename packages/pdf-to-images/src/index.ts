...
function normalizeInput(input: ConvertInput): string | Buffer {
  ...
  throw new PdfToImagesError("INVALID_INPUT", "Expected a file path, Buffer, Uint8Array, or ArrayBuffer.");
}
...
function normalizeCrop(crop: Crop | undefined): Crop | undefined {
  ...
  if (values.some((value) => !Number.isFinite(value))) {
    throw new PdfToImagesError("INVALID_CROP", "Crop values must be finite numbers.");
  }

  if (crop.x < 0 || crop.y < 0 || crop.width <= 0 || crop.height <= 0) {
    throw new PdfToImagesError("INVALID_CROP", "Crop values must be non-negative and width/height must be greater than zero.");
  }
...
function normalizeOptions(options: ConvertOptions = {}): ConvertOptions {
  if (options.outputFormat && options.outputFormat !== "png") {
    throw new PdfToImagesError("INVALID_OPTIONS", "Only PNG output is supported in v1.");
  }

  if (options.dpi !== undefined) {
    if (!Number.isFinite(options.dpi) || options.dpi <= 0) {
      throw new PdfToImagesError("INVALID_OPTIONS", "DPI must be a positive number.");
    }
  }

  if (options.pages) {
    if (!Array.isArray(options.pages)) {
      throw new PdfToImagesError("INVALID_OPTIONS", "`pages` must be an array of zero-based page indices.");
    }

    for (const pageIndex of options.pages) {
      if (!Number.isInteger(pageIndex) || pageIndex < 0) {
        throw new PdfToImagesError("INVALID_PAGE_INDEX", "Page indices must be non-negative integers.");
      }
    }
  }
...
function mapNativeError(error: unknown): never {
  ...
  if (error instanceof Error) {
    ...
    if (separatorIndex > 0) {
      const code = error.message.slice(0, separatorIndex) as PdfToImagesErrorCode;
  ...
  throw new PdfToImagesError("RENDER_ERROR", "Unknown native error.", { cause: error });
}

export async function convert(input: ConvertInput, options: ConvertOptions = {}): Promise<ConvertedPage[]> {
  const normalizedInput = normalizeInput(input);
  const normalizedOptions = normalizeOptions(options);

  try {
    if (typeof normalizedInput === "string") {
      return await nativeBinding.convertPath(normalizedInput, normalizedOptions, bundledPdfiumPath);
    }

    return await nativeBinding.convertBytes(normalizedInput, normalizedOptions, bundledPdfiumPath);
  } catch (error) {
